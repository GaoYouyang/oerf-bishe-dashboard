...
| 代码健康检查 | `run.py` 硬编码 `cuda:3`；投影扩展是 Linux x86-64 `.so`；trainer import 即读 `/home/hyz/...`；requirements 有重复 pyDOE、三套 OpenCV 和 `skimage==0.0`；fuel test 路径与 train 全重叠 | 公开快照不能写成已验证的一键运行；只能结构级学习并做 clean-room baseline |
