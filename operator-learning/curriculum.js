window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.25-c-semiconvergence-coverage-audit-rerun",
  updated: "2026-07-25",
  foundationChecks: [
    {
      id: "python-array",
  ...
  resources: [
    ...
    {id:"poolfire-c-semiconvergence-coverage",stage:"warm-start",level:"必做",type:"最新算法诊断",title:"C 路线半收敛、Warm Start 负结果与覆盖扩充门",url:"../document_reader.html?doc=docs%2Fpoolfire_c_semiconvergence_coverage_diagnosis_2026-07-25.md",local:"",read:"先比较 p22 Zero K4/K24 的 field、gradient、observation 三条曲线，再看 ridge K2 固定迁移为何在 p14 与 wall time 失败，最后读 v2 线性摊销失败、v3 PCA coverage failure 和 v4 两轨迹分阶段门。",output:"不用页面解释：为什么 residual 继续下降仍可能重建更差；为什么 37.5% 少调用不是速度成功；为什么 fit PCA 能量接近 100% 仍不能授权 FNO。",verified:"独立审计后 73 passed 并重跑；五轨迹四臂 classical 开发汇总完成；v2 FAIL_NO_RANK_MATCHES_P14_CGLS_K4_AT_ZERO_DEPTH 且 p22 未读；v3 FAIL_LATENT_OUTPUT_PCA_HEADROOM；v4 只先扩 p14s05/p22s03；test_truth_opened=false；algorithm_breakthrough=false"},
  ...
  researchTracks: [
    {
      id:"warmstart-c0", rank:1, title:"C0 · Adjoint-Residual Warm Start", badge:"唯一主线的最小闭环", risk:"低-中", novelty:"创新性不是模型名字，而是 BOST 的 geometry-conditioned initialization、半收敛感知 physical refinement、calibration-aware correction budget 与严格 matched-accuracy 成本证据", data:"PoolFire Δrho/Δn 使用 validation 前冻结的 reference/gauge，并按完整 trajectory/工况切分；五条开放轨迹 classical 表已完成，两条 untouched test 仍封存；当前先用 p14s05/p22s03 扩充 clean-fit coverage，后续再接独立公开或组内光学样例", hardware:"当前 16×16×32 ridge/solver/PCA 闭环可在 Mac 秒级完成，PyTorch MPS 可用于最小模型；只有 coverage gate 改善后才训练 3D U-Net/FNO/UNO，届时再按显存与吞吐决定是否租 GPU", question:"一次 observation/A^T y 或参考态 J^T residual 生成的 x0，能否用 validation 冻结的半收敛安全区保持终点等价并减少物理调用和总时间？", contribution:"把 direct operator 变成可被经典求解器少量纠正、但不会被错误 forward 过度拉坏的低成本起点，并分开报告 deployable 主账、oracle headroom、p90 与 harm rate。", next:"接入 p14s05/p22s03→原样重跑 pair/K4/PCA coverage gate→改善至少 20% 后训练最小 U-Net/FNO/UNO→部署可见 correction/fallback gate。", stop:"新增 clean-fit 后 representation coverage 不改善、ridge/FNO headroom 消失、网络推理与特征成本超过所省迭代、终点精度变差或 p90/harm 变坏时停止。"
    },
    {
