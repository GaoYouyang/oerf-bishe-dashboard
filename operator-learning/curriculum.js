window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.26-c-v8-centroid-calibration-fail-full-field-next",
  updated: "2026-07-26",
  foundationChecks: [
    {
      id: "python-array",
  ...
  researchTracks: [
    {
      id:"warmstart-c0", rank:1, title:"C0 · Adjoint-Residual Warm Start", badge:"唯一主线的最小闭环", risk:"低-中", novelty:"创新性不是模型名字，而是 BOST 的 geometry-conditioned initialization、半收敛感知 physical refinement、calibration-aware correction budget 与严格 matched-accuracy 成本证据", data:"PoolFire Δrho/Δn 使用 validation 前冻结的 reference/gauge，并按完整 trajectory/工况切分；五条开放轨迹 classical 表、v4/v5.1 表示门、v6/v7.1 定位门和 v8 低容量质心校准门均完成，两条 untouched test 仍封存；shifted-POD 质心代理路线已经停止", hardware:"当前 16×16×32 solver 与低容量空间基线可在 Mac 上开发；尚未授权大模型训练或租用 GPU。先比较 identity/equalized BP、局部 separable 3D convolution/ridge 和低模频域 transfer，只有完整轨迹 full-field headroom 稳定后才训练一个小型 BP-conditioned 3D U-Net sentinel", question:"一次 observation/A^T y 或参考态 J^T residual 生成的 x0，能否用 validation 冻结的半收敛安全区保持终点等价并减少物理调用和总时间？", contribution:"把 direct operator 变成可被经典求解器少量纠正、但不会被错误 forward 过度拉坏的低成本起点，并分开报告 deployable 主账、oracle headroom、p90 与 harm rate。", next:"结果前冻结 observation/BP→完整16×16×32 warm field 合同→依次比较 identity、equalized、局部 separable 3D convolution/ridge、低模频域 transfer→按完整 trajectory 检查 full-field/gradient/observation headroom 与 harm→通过后只训练一个小型 BP-conditioned 3D U-Net sentinel→再决定是否比较 FNO/UNO/DeepONet。", stop:"低容量 full-field controls 没有跨轨迹稳定 headroom、任何方法依赖 heldout truth 选择、sentinel 的 matched-accuracy/harm/calls/wall/RSS 任一主门失败、网络推理与特征成本超过所省迭代、终点精度变差或尾部伤害变坏时停止。"
    },
    {
