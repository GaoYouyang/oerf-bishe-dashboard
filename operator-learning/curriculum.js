window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.25-c-v4-identity-isolation-sentinel-gate",
  updated: "2026-07-25",
  foundationChecks: [
    {
      id: "python-array",
  ...
  resources: [
    ...
    {id:"poolfire-c-semiconvergence-coverage",stage:"warm-start",level:"必做",type:"最新算法诊断",title:"C 路线半收敛、Warm Start 负结果与覆盖扩充门",url:"../document_reader.html?doc=docs%2Fpoolfire_c_semiconvergence_coverage_diagnosis_2026-07-25.md",local:"",read:"先比较 p22 Zero K4/K24 的 field、gradient、observation 三条曲线，再看 ridge K2 固定迁移为何在 p14 与 wall time 失败，最后读 v2 线性摊销失败、v3 PCA coverage failure、v4 两轨迹数据释放门和 sentinel 前置门。",output:"不用页面解释：为什么 residual 继续下降仍可能重建更差；为什么 37.5% 少调用不是速度成功；为什么 fit PCA 能量接近 100% 仍不能授权 FNO；为什么 v4 相对改善 20% 只允许继续接数据。",verified:"两轮独立审计后完整 PoolFire 回归 198 passed；五轨迹四臂 classical 开发汇总完成；v2 FAIL_NO_RANK_MATCHES_P14_CGLS_K4_AT_ZERO_DEPTH 且 p22 未读；v3 FAIL_LATENT_OUTPUT_PCA_HEADROOM；v4 显式排除 p22 manifest 并只先扩 p14s05/p22s03；20% 门不授权神经训练；test_truth_opened=false；algorithm_breakthrough=false"},
