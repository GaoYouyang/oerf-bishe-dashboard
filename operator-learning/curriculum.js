window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.26-c-v9-three-role-gate-loto-next",
  updated: "2026-07-26",
  foundationChecks: [
    {
      id: "python-array",
  ...
  researchTracks: [
    {
      id:"warmstart-c0", rank:1, title:"C0 · Adjoint-Residual Warm Start", badge:"唯一主线的最小闭环", risk:"低-中", novelty:"创新性不是模型名字，而是 BOST 的 geometry-conditioned initialization、半收敛感知 physical refinement、calibration-aware correction budget 与严格 matched-accuracy 成本证据", data:"PoolFire Δrho/Δn 使用 validation 前冻结的 reference/gauge，并按完整 trajectory/工况切分；五条开放轨迹 classical 表、v4-v8 机制门、v9 结果前协议、Cross14 核心和 synthetic-only 三角色代码门均完成；p45-s03 与两条 untouched test 仍封存", hardware:"Cross14 只有 14 个 float64 权重，16×16×32 核心与 fit/deployment/score 合成代码门已在 Mac 上通过。当前先实现 nested LOTO/one-SE 外层编排器和 clean detached whole-pipeline harness，再运行五条 101 帧、强基线与 wall/RSS；只有完整轨迹门通过才授权小型 BP-conditioned 3D U-Net sentinel", question:"一次 observation/A^T y 或参考态 J^T residual 生成的 x0，能否用 validation 冻结的半收敛安全区保持终点等价并减少物理调用和总时间？", contribution:"把 direct operator 变成可被经典求解器少量纠正、但不会被错误 forward 过度拉坏的低成本起点，并分开报告 deployable 主账、oracle headroom、p90 与 harm rate。", next:"实现绑定 outer/inner complete-trajectory LOTO、完整 lambda 网格、one-SE、训练数组语义 provenance 与 clean commit 的 formal orchestrator→生成五条 101 帧三角色请求→完成 p14 veto 与全部 controls→测 whole-pipeline wall/RSS→只有全部 development 门过才一次性释放 p45-s03。", stop:"Cross14 或低容量 controls 没有跨轨迹稳定 headroom、任一方法依赖 heldout truth 选择、matched-accuracy/harm/calls/wall/RSS 任一主门失败、网络推理与特征成本超过所省迭代、终点精度变差或尾部伤害变坏时停止。"
    },
    {
