window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.23-c-real-cfd-morphology-proxy",
  updated: "2026-07-23",
  foundationChecks: [
    {
      id: "python-array",
  ...
  weeks: [
    ...
    },
    {
      ...
      resources: ["c-route-lock", "poolfire-rho-bridge", "poolfire-preprocessing-proxy", "poolfire-optical-g0", "poolfire-tier-a-straight", "poolfire-cell-center-gate", "poolfire-reference-forward-gate", "poolfire-cfd-morphology-proxy", "poolfire-data", "nerif-paper", "he-data-contract", "pytorch-autograd"],
    ...
    },
    {
      ...
      build: ["复现已通过的统一 baseline contract gate", "复现真实 PoolFire CFD 形态代理闭环", "按 train / ridge-selection / refinement-validation / later-time 四段切分", "跑 Zero-CGLS、归一化 BP-CGLS 与 ridge Direct-Warm", "冻结固定 refinement depth 并分别输出 deployable 主账与 post-hoc headroom"],
      pass: ["统一底座与真实代理相关测试保持 23 PASS，one-shot cache 运行后归零", "高分辨率 reference 与粗 inverse 不共享离散矩阵", "零初值 K 步调用账与公式一致", "非零初值额外 forward 明确记账", "ridge 选择与 refinement-depth 选择使用不同 purged time blocks", "test truth 只事后画 headroom，不能控制在线 solver", "明确披露 v0 later-time frames 已打开、fresh confirmatory 仍为 0"],
      resources: ["c-route-lock", "poolfire-c-baseline-contract", "poolfire-cfd-morphology-proxy", "fair-budget-audit", "v3k-d-controls", "v3k-f-stopping", "discrepancy-hanke"],
  ...
  researchTracks: [
    {
      id:"warmstart-c0", rank:1, title:"C0 · Adjoint-Residual Warm Start", badge:"唯一主线的最小闭环", risk:"低-中", novelty:"创新性不是模型名字，而是 BOST 的 geometry-conditioned initialization、物理 refinement、calibration-aware correction budget 与严格 matched-accuracy 成本证据", data:"PoolFire Δrho/Δn 使用 validation 前冻结的 reference/gauge，并按完整 trajectory/工况切分；当前单轨迹只作 development，新增 trajectory 才能承担 fresh test；后续再接独立公开或组内光学样例", hardware:"当前 16×16×32 ridge/solver 闭环可在 Mac 秒级完成；多 trajectory 的最小 FNO/UNO 可先本机小规模，确认 headroom 后再决定 GPU", question:"一次 observation/A^T y 或参考态 J^T residual 生成的 x0，能否用 validation 冻结的 correction budget 保持终点等价并减少物理调用和总时间？", contribution:"把 direct operator 变成可被经典求解器少量纠正、但不会被错误 forward 过度拉坏的低成本起点，并分开报告 deployable 主账、oracle headroom、p90 与 harm rate。", next:"下载新增 PoolFire trajectories→冻结 fresh trajectory split→复现 ridge K=2 信号→最小 FNO/UNO 对照→部署可见 correction gate。", stop:"新增 trajectory 上 ridge/FNO headroom 消失、网络推理与特征成本超过所省迭代、终点精度变差或 p90/harm 变坏时停止。"
    },
    {
