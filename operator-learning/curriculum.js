window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.28-c-v24-external-proxy-replication",
  ...
  researchTracks: [
    ...
    {
      id:"warmstart-c1", rank:2, title:"C1 · Observable Reduced Warm K1", badge:"v24 external proxy · 3/3 PASS", risk:"中", novelty:"51 个偶数帧运行紧凑 full-view dual proposal 与精确 A^T lift；50 个奇数帧只用部署可见 observation residual 在 fit-only rank-199 包内恢复 warm lift，随后进入同一 strict CGLS K1。", data:"三条结果前冻结、此前不在项目数据根目录的公开 PoolFire train-split 轨迹逐条通过：p33-s05 与 p45-s01 为 101/101 joint match，p58-s05 为 99/101；三条 harm=0。它们是新功率/尺寸组合，但不是 unseen-power、unseen-size、geometry OOD、官方盲测或真实 BOST。", hardware:"每条 101 帧在线账 202A+152A^T=354，父模型 404，Zero-K4 808；fresh-process wall 分别快 13.48%/11.81%/12.80%，RSS p90 ratio 1.0161/1.0123/1.0198，只支持 memory no-harm。离线 package 400 次调用，按等成本需 8 条序列摊平。", question:"固定方法迁移到组内真实 BOST 后，能否在重复测量与标定不确定度定义的同精度门内，仍保留完整 A/A^T 与端到端 wall 优势？", contribution:"首次获得三条外部公开 CFD proxy 的逐轨迹精度、harm、完整调用、paired wall、RSS 和独立复算共同通过；候选最大复算差不超过 2.20e-15。当前是重要 proxy replication milestone，algorithm_breakthrough=false。", next:"停止追加同类 CFD 工况，直接准备组内真实 BOST 迁移：重复采集、flow-off、相机标定、真实 forward/adjoint、组内基线和端到端资源账。", stop:"真实 BOST 任一重复实验发生明显 harm、wall 优势消失、离线 setup 无法摊销或被更便宜基线支配，就降级为公开 CFD proxy 方法，不以改大模型挽救。"
    },
    {
