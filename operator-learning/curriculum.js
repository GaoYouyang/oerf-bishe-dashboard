window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.25-c-v6-raw-bp-transport-proxy-fail",
  updated: "2026-07-25",
  foundationChecks: [
    {
      id: "python-array",
  ...
  researchTracks: [
    {
      id:"warmstart-c0", rank:1, title:"C0 · Adjoint-Residual Warm Start", badge:"唯一主线的最小闭环", risk:"低-中", novelty:"创新性不是模型名字，而是 BOST 的 geometry-conditioned initialization、半收敛感知 physical refinement、calibration-aware correction budget 与严格 matched-accuracy 成本证据", data:"PoolFire Δrho/Δn 使用 validation 前冻结的 reference/gauge，并按完整 trajectory/工况切分；五条开放轨迹 classical 表、v4/v5.1 表示门与 v6 raw-BP 输运定位门均完成，两条 untouched test 仍封存；当前不继续接剩余同类 fit", hardware:"当前 16×16×32 solver 与固定几何 D^{-1}A^T y 审计可在 Mac 秒到分钟级完成；尚未授权神经训练或租用 GPU。只有 deployment-visible 定位通过逐轨迹门，并重新冻结独立验证协议后，才运行 aligned containment；再有明确 headroom 才训练一个小型 3D U-Net sentinel", question:"一次 observation/A^T y 或参考态 J^T residual 生成的 x0，能否用 validation 冻结的半收敛安全区保持终点等价并减少物理调用和总时间？", contribution:"把 direct operator 变成可被经典求解器少量纠正、但不会被错误 forward 过度拉坏的低成本起点，并分开报告 deployable 主账、oracle headroom、p90 与 harm rate。", next:"冻结几何灵敏度均衡 D^{-1}A^T y→按同一六轨迹 T0 门比较 raw/equalized BP→全部过门才运行 zero-fill aligned containment→有实质 headroom 才做 conditional basis / mixture→再冻结 trajectory-level confirmatory 门→单个 BP-conditioned 3D U-Net sentinel。", stop:"均衡 BP 仍不能逐轨迹可靠定位、shift 丢失能量或依赖 target oracle、sentinel 的 matched-accuracy/harm/calls/wall/RSS 任一主门失败、网络推理与特征成本超过所省迭代、终点精度变差或尾部伤害变坏时停止。"
    },
    {
