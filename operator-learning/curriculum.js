window.OPERATOR_LEARNING_GUIDE = {
  version: "2026.07.26-c-v9-cross14-core-gate-process-next",
  updated: "2026-07-26",
  foundationChecks: [
    {
      id: "python-array",
  ...
  researchTracks: [
    {
      id:"warmstart-c0", rank:1, title:"C0 · Adjoint-Residual Warm Start", badge:"唯一主线的最小闭环", risk:"低-中", novelty:"创新性不是模型名字，而是 BOST 的 geometry-conditioned initialization、半收敛感知 physical refinement、calibration-aware correction budget 与严格 matched-accuracy 成本证据", data:"PoolFire Δrho/Δn 使用 validation 前冻结的 reference/gauge，并按完整 trajectory/工况切分；五条开放轨迹 classical 表、v4-v8 机制门、v9 结果前协议和 Cross14 无数据核心代码门均完成；p45-s03 与两条 untouched test 仍封存", hardware:"Cross14 只有 14 个 float64 权重，16×16×32 核心和 exact-K2 已在 Mac 上通过 31 项单测，不需要租 GPU。当前先完成 fit/deployment/score 三进程、geometry/operator provenance 和 whole-pipeline wall/RSS；只有完整轨迹门通过才授权小型 BP-conditioned 3D U-Net sentinel", question:"一次 observation/A^T y 或参考态 J^T residual 生成的 x0，能否用 validation 冻结的半收敛安全区保持终点等价并减少物理调用和总时间？", contribution:"把 direct operator 变成可被经典求解器少量纠正、但不会被错误 forward 过度拉坏的低成本起点，并分开报告 deployable 主账、oracle headroom、p90 与 harm rate。", next:"冻结 fit/deployment/score manifest 与文件角色→绑定 protocol、代码、trajectory、geometry/equalizer、solver、runtime 和报告模板→完成五条 outer LOTO 与 p14 veto→公平比较 Cross14-K2、Zero-K3/K4 和全部 controls→只有 development、wall、RSS 全过才一次性释放 p45-s03→通过后才预注册小型 3D U-Net sentinel。", stop:"Cross14 或低容量 controls 没有跨轨迹稳定 headroom、任一方法依赖 heldout truth 选择、matched-accuracy/harm/calls/wall/RSS 任一主门失败、网络推理与特征成本超过所省迭代、终点精度变差或尾部伤害变坏时停止。"
    },
    {
