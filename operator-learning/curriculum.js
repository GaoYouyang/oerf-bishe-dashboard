window.OPERATOR_LEARNING_GUIDE = {
  ...
  researchTracks: [
    ...
    {
      id:"warmstart-c1", rank:2, title:"C1 · Compact Full-view DualRange-K1", badge:"v18 temporal FAIL · 94/101 scheduled", risk:"中-高", novelty:"让完整三视图 observation 通过奇对称紧凑 detector CNN 生成 full-view dual；精确 A^T lift、解析 alpha 和 strict K1 保持不变。当前参数从 77,020 压到 10,548。", data:"五条 fit trajectory LOTO 均 PASS；v13 资源预检没有读取新的 fit、fresh、validation、test 或真实 BOST 数据。v11.3 的一次性 p45 fresh 与 v16 的额外 p33 development 都达到 101/101、0 harm、调用减半，两条以后只能作 development。v18 只在已消费 p33 上做 post-open mechanism diagnostic，没有打开新的 fresh、validation、test 或真实 BOST。", hardware:"v17 的 wall 中位快 11.06%、RSS p90 ratio 0.99488，但 child CPU ratio 1.11277、第一次冷进程 harm 236.82%。v18 尝试少运行 CNN，却在 p33 仍计划执行 94/101 次，只减少 6.93%；这是 counterfactual 调度账，没有运行原生 wall/CPU/RSS profile。", question:"简单 observation-change gate 与 previous-dual/diagonal-delta transport 已被正式否定；剩余科学问题是显式运动/拓扑状态能否跨 trajectory 预测何时可以安全复用。", contribution:"已形成多视图 detector-space dual proposal、精确 Range(A^T) lift、短程 Krylov、跨轨迹精度证据和严格资源边界；但稳定 CPU/冷启动优势、可泛化时间复用与真实 BOST 尚未成立。", next:"只有显式运动或拓扑模型同时打赢 previous-field、周期关键帧、Krylov recycling 和同 CNN 预算控制，才值得再开正式门；否则停止 proxy-only 时间支线。", stop:"若 trajectory-level LOTO 不能同时通过 compatibility 与至少 20% 原生 CNN 减少，就记负结果，不做阈值追调或换大模型挽救。"
    },
    {
