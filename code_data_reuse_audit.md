...
| `Hyz617/TDBOST` | 2026-07-10 API：main tree `3393ca7`，59 blobs / 2,287,454 bytes，no license、无 release；含 configs/dataloader/TDmodel/render/run/projection。运行入口硬编码 `cuda:3`，Linux x86-64 `.so`、`/home/hyz/...` import-time 路径与冲突 requirements 使公开快照不能直接一键复现；fuel test 18 路径全部重叠 train，spray 162/18 split 才不重叠 | 只读结构：rank、config、data loader、projection、render、run loop；先做 manifest overlap/view-time coverage 审计和 clean-room M3B；详见 `tdbost_reproducibility_audit.html` | 不直接复制代码/数据，不把 README 的 clone-install-run 当已验证流程；运行和代码级复用先问何远哲与许可 |
