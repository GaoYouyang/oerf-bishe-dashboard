...
def test_v246_is_latest_on_bilingual_primary_pages() -> None:
    current = json.loads(CURRENT.read_text(encoding="utf-8"))
    assert current["scientific_status"] == "FAIL_CASE19_TWO_IMPLEMENTATION_ENVELOPE_NOT_MONOTONE_V246"
    ...
    assert "v246 Case 19 参考收敛判决已封存" in focus
    assert "v246 Case 19 reference-convergence verdict sealed" in focus
