...
SUMMARY = ROOT / "docs/blastnet_case5_local_ray_coverage_attribution_v211_public_summary.json"
RESULT = ROOT / "docs/blastnet_case5_local_ray_coverage_attribution_v211_result_2026-08-23.md"
FIGURE = ROOT / "assets/figures/blastnet_case5_local_ray_coverage_attribution_v211.png"
CURRENT = ROOT / "operator-learning/current-evidence.json"


def test_v211_public_summary_preserves_the_strict_negative_decision() -> None:
    ...
    assert primary["virtual_strictly_greater_count"] == 0
    assert primary["tie_count"] == 0
    assert primary["superiority_fraction"] == 0.0
    ...
    assert virtual["primary_min_median_max"][2] < supplied["primary_min_median_max"][0]
    assert virtual["local_floor_median_min_median_max"][1] > supplied["local_floor_median_min_median_max"][1]


def test_v211_public_summary_preserves_independence_and_zero_science_reads() -> None:
    payload = json.loads(SUMMARY.read_text())
    ...
    assert validation["maximum_local_eigenvalue_difference"] < 1e-12
    assert validation["maximum_camera_reversal_difference"] == 0
    scope = payload["scope"]
    assert scope["geometry_rows"] == 39
    assert scope["interior_tensor_count_per_geometry"] == 5880
...
def test_v211_result_and_figure_are_bilingual_and_nonblank() -> None:
    text = RESULT.read_text()
    assert "# v211：" in text and "# v211:" in text
    assert "0/169" in text and "169/169" in text
    assert "FAIL_LOCAL_RAY_COVERAGE_DOES_NOT_EXPLAIN_CASE5_REFERENCE_V211" in text
...
def test_v211_is_preserved_as_parent_evidence_after_v212() -> None:
    ...
    assert current["metrics"]["v211_primary_opposite_direction_count"] == 169
    assert current["current_decision"]["v211_expected_direction_supported"] is False
    assert current["current_decision"]["v211_all_local_geometry_irrelevant"] is False
    assert current["current_decision"]["v211_predictor_authorized"] is False
    assert current["v211_local_ray_coverage_independent_status"] == (
        "PASS_INDEPENDENT_RECOMPUTATION_LOCAL_RAY_COVERAGE_ATTRIBUTION_V211"
    )
    assert "blastnet_case5_local_ray_coverage_attribution_v211" in (
        ROOT / "operator-learning/index.html"
    ).read_text()


def test_primary_pages_reference_v211_in_both_languages() -> None:
    for path in (
        ROOT / "index.html",
        ROOT / "operator-learning/index.html",
        ...
        assert "blastnet_case5_local_ray_coverage_attribution_v211" in content
        assert "algorithm_breakthrough=false" in content
    focus = (ROOT / "operator-learning/index.html").read_text()
    assert "v211 Case 5：局部射线覆盖下尾" in focus
    assert "v211 Case 5: the local ray-coverage lower tail" in focus


def test_v211_public_artifacts_contain_no_private_execution_material() -> None:
    forbidden = [
        "formal_commit",
        "validator_commit",
        '"run_id"',
