#!/usr/bin/env python3
"""Build the public v174 equal-cost selector attribution figure."""
...
OUTPUT = ROOT / "assets/figures/real_bost_equal_cost_selector_attribution_v174.png"
WIDTH = 2520
HEIGHT = 1320
...
def main() -> int:
    ...
    order = ["v172_selector", "ray_axis_maximin", "fit_static", "v169_low_mode_d_opt"]
    labels = {
        "v172_selector": "v172 selector",
        "ray_axis_maximin": "Ray-axis maximin",
        "fit_static": "Fit-static",
        "v169_low_mode_d_opt": "v169 low-mode D-opt",
    }
    colors = {
        "v172_selector": "#267a68",
    ...
    _text(
        draw,
        (WIDTH / 2, 58),
        "v174 equal-cost camera-selector attribution",
        size=46,
        fill="#17232d",
        bold=True,
        anchor="mm",
    ...
    _text(
        draw,
        (WIDTH / 2, 114),
        "same H1-K0 reconstruction | same 1A + 1A^T ledger | independent recomputation",
        size=23,
        fill="#596772",
        anchor="mm",
    )
    ...
    _text(
        draw,
        (1260, 1120),
        "Independent verdict: v172 is the only complete policy at identical H1-K0 and 1A + 1A^T cost",
        size=25,
        fill="#245f83",
        bold=True,
        anchor="mm",
    ...
    _text(
        draw,
        (1260, 1173),
        "468/468 cells | 13/13 calibrations | 9/9 fields | 4/4 times | 27/27 independent checks",
        size=21,
        fill="#4f5f68",
        anchor="mm",
    )
    _text(
        draw,
        (1260, 1218),
        "PASS_POSTOPEN_SELECTOR_ONLY_HEADROOM_V174 | algorithm_breakthrough=false",
        size=20,
        fill="#267a68",
        bold=True,
        anchor="mm",
