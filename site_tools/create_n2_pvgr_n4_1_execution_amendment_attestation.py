#!/usr/bin/env python3
"""Create the one-time attestation for the N4.1 execution amendment."""
...
def build_attestation(
    ...
    *,
    ...
    if current_config != frozen_config:
        raise RuntimeError("current N4.1 config differs from protocol commit")
    ...
    if output_path.resolve() != expected_output:
        raise ValueError("output path must match N4.1 pre_registration_attestation")
    if output_path.exists():
        raise FileExistsError("N4.1 attestation already exists")
    formal_output = (ROOT / config["formal_output"]).resolve()
    formal_work = (ROOT / config["formal_work_output"]).resolve()
    if formal_output.exists():
        raise FileExistsError("N4.1 formal output exists before attestation")
    if formal_work.exists():
        raise FileExistsError("N4.1 formal work exists before attestation")

    files: dict[str, dict[str, Any]] = {}
    for key, relative in config["attested_files"].items():
        path = (ROOT / relative).resolve()
        if ROOT not in path.parents:
            raise ValueError(f"N4.1 attested file escapes repository: {relative}")
        current = path.read_bytes()
        frozen = _git("show", f"{protocol_commit}:{relative}", binary=True)
        if current != frozen:
            raise RuntimeError(f"N4.1 file differs from protocol commit: {relative}")
        files[str(key)] = {
            "path": relative,
    ...
    attestation = {
        "schema": "n2-pvgr-n4-1-execution-amendment-attestation-1.0",
        ...
        "claim_boundary": (
            "This attests the disclosed N4.1 control-flow repair before any N4.1 "
            "checkpoint; the frozen N4 scientific samples and gates are unchanged."
        ),
    }
