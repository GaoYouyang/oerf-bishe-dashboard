#!/usr/bin/env python3
"""Build the public v164.1 affine-transport decision matrix."""
...
OUTPUT = ROOT / "assets/figures/real_bost_observation_affine_transport_v164_1.png"
WIDTH = 2520
HEIGHT = 1180
...
def main() -> int:
    ...
    rows = {
        (row["time"], row["camera_count"]): row for row in payload["primary"]["strata"]
    }
    ...
    _text(
        draw,
        (WIDTH / 2, 62),
        "v164.1 observation-fitted global affine transport",
        size=44,
        fill="#17232d",
        bold=True,
        anchor="mm",
    ...
    gap_x, gap_y = 34, 30
    for column, camera_count in enumerate(cameras):
    ...
    draw.rounded_rectangle(
        (230, 1050, 2290, 1133), radius=8, fill="#ffffff", outline="#d7e0e7", width=2
    )
    _text(
        draw,
        (WIDTH / 2, 1077),
        "Independent verdict: 10/12 pass; global affine transport does not rescue the two five-camera tails.",
        size=25,
        fill="#7a3f2d",
        bold=True,
        anchor="mm",
    ...
    _text(
        draw,
        (WIDTH / 2, 1112),
        "t=0.75: 0.789 / 1.078 | non-anchor cost 13A+1AT | 39/39 checks pass | mechanism closed",
        size=21,
        fill="#66737e",
        anchor="mm",
    )
