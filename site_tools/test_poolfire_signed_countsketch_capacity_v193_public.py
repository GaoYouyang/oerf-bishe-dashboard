...
def test_v193_public_assets_and_bilingual_claims_exist() -> None:
    ...
    assert "curriculum.js?v=20260822-v193-1" in focus
...
def test_current_evidence_points_to_v193_without_overclaiming() -> None:
    current = json.loads((ROOT / "operator-learning/current-evidence.json").read_text(encoding="utf-8"))
    assert current["scientific_status"] == "FAIL_SIGNED_COUNTSKETCH_CAPACITY_V193"
