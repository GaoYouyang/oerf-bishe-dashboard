...
STEM = 'poolfire_spectral_camera_precision_20260907'


def test_result_and_budget_not_capacity_impossibility():
    d = json.loads((ROOT/'docs'/f'{STEM}.json').read_text())
    assert d['status'] == 'FAIL_OPENED_CAMERA_PRECISION_SUM_LOTO'
    assert d['verification']['status'] == 'PASS_INDEPENDENT_CAMERA_PRECISION_LOTO'
    assert len(d['reports']) == 22 and len(d['fits']) == 10
    ...
    assert d['comparisons'][0]['any_harm_cells'] == 505 and not d['fairness']
...
def test_bilingual_claims_and_redacted_files():
    ...
    assert all('87' in current[f'next_scientific_gate_{lang}'] for lang in ('zh', 'en'))
    for rel in ('index.html', 'operator-learning/index.html', 'operator-learning/daily-progress.html'):
        ...
        notes = soup.select('#spectral-precision-result')
        assert len(notes) == 1
        for lang in ('zh', 'en'):
            assert all(v in notes[0][f'data-i18n-{lang}'] for v in ('87', '0/505', '0/5', '1%', '200'))
...
def test_instrumented_ledger_and_old_reports_preserved():
    d = json.loads((ROOT/'docs'/f'{STEM}.json').read_text())
    fits = sum(r['optimizer_evaluations']+1 for r in d['fits'])
    assert d['formal_offline_calls']['fit_A'] == d['formal_offline_calls']['fit_AT'] == 808*fits
    assert d['independent_additional_offline_calls']['post_query_A'] == 2020
    prior = json.loads((ROOT/'docs/poolfire_camera_mix_ablation_20260907.json').read_text())
    assert prior['stage_attribution']['status'] == 'P33_INTERIOR_HARM_PREEXISTS_K1'
