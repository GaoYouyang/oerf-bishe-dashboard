...
def test_v238_is_synchronized_as_the_latest_public_decision() -> None:
    ...
    assert current["public_evidence"]["figure"].endswith(
        "blastnet_case7_octant_tail_subspace_capacity_v238.png"
    )
    ...
    assert log.index("v238") < log.index("v236")
