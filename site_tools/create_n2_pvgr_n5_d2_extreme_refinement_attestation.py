#!/usr/bin/env python3
"""Create the one-time attestation for the N5-D2 extreme-refinement probe."""
...
def build_attestation(
    ...
    *,
    ...
    if str(_git("rev-parse", "HEAD")) != protocol_commit:
        raise RuntimeError("N5-D2 attestation must be created at the protocol commit")
    ...
    if current_config != frozen_config:
        raise RuntimeError("current N5-D2 config differs from protocol commit")
    ...
    if output_path.resolve() != expected_output:
        raise ValueError("output path must match N5-D2 attestation path")
    if output_path.exists():
        raise FileExistsError("N5-D2 attestation already exists")
    formal_output = (ROOT / config["formal_output"]).resolve()
    if formal_output.exists():
        raise FileExistsError("N5-D2 formal output exists before attestation")

    files: dict[str, dict[str, Any]] = {}
    for key, relative in config["attested_files"].items():
        path = (ROOT / relative).resolve()
        if ROOT not in path.parents:
            raise ValueError(f"N5-D2 attested file escapes repository: {relative}")
        current = path.read_bytes()
        frozen = _git("show", f"{protocol_commit}:{relative}", binary=True)
        if current != frozen:
            raise RuntimeError(f"N5-D2 file differs from protocol commit: {relative}")
        files[str(key)] = {
            "path": relative,
    ...
    attestation = {
        "schema": "n2-pvgr-n5-d2-extreme-refinement-attestation-1.0",
        ...
        "claim_boundary": (
            "This attests that the four post-D1 cells, H4096/H8192 levels, "
            "paired-Neumaier observable, contraction and tail gates, runner, "
            "tests and independent validator were committed before D2 output existed."
        ),
    }
