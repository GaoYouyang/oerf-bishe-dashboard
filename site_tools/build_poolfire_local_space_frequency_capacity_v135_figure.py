...
SUMMARY = ROOT / "docs/poolfire_local_space_frequency_capacity_v135_public_summary.json"
OUTPUT = ROOT / "assets/figures/poolfire_local_space_frequency_capacity_v135.png"
...
def build_figure() -> None:
    ...
    stage_labels = ["v133\nGlobal bands", "v134\nPareto weights", "v135\nLocal 2x2"]
    stage_values = [gate["v133_passed"], gate["v134_passed"], gate["v135_passed"]]
    stage_colors = ["#8c999f", "#4d8d7c", "#2b6f62"]
    bars = cascade_ax.bar(np.arange(3), stage_values, color=stage_colors, width=0.68)
    ...
    cascade_ax.set_title("A. Strict capacity rises, but remains incomplete", loc="left", fontsize=12.8, weight="bold")
    cascade_ax.grid(axis="y", alpha=0.18)
    for bar, value in zip(bars, stage_values, strict=True):
        cascade_ax.text(bar.get_x() + bar.get_width() / 2, value + 70, str(value), ha="center", weight="bold")
    ...
    v134 = [by_camera[count]["v134_passed"] for count in camera_counts]
    v135 = [by_camera[count]["v135_passed"] for count in camera_counts]
    width = 0.34
    camera_ax.bar(x - width / 2, v134, width, label="v134", color="#8c999f")
    camera_ax.bar(x + width / 2, v135, width, label="v135 local 2x2", color="#2b6f62")
    ...
    camera_ax.set_title("B. Sparse five-camera views dominate failures", loc="left", fontsize=12.8, weight="bold")
    camera_ax.grid(axis="y", alpha=0.18)
    camera_ax.legend(frameon=False, loc="upper left")
    for i, count in enumerate(camera_counts):
        camera_ax.text(i + width / 2, v135[i] + 20, f"{v135[i]}", ha="center", fontsize=9.5, weight="bold")
    ...
    trajectory_ax.set_ylim(0, 280)
    trajectory_ax.set_ylabel("Remaining observation-only failures")
    trajectory_ax.set_title("C. p45 and p58 dominate the unresolved tail", loc="left", fontsize=12.8, weight="bold")
    ...
    tail_ax.axhline(1.05, color="#26383f", linestyle="--", linewidth=1.4, label="per-cell gate 1.05")
    ...
    tail_ax.set_title("D. Only observation tails remain", loc="left", fontsize=12.8, weight="bold")
    tail_ax.grid(axis="y", alpha=0.18)
    tail_ax.legend(frameon=False, loc="upper left")

    fig.suptitle(
        "v135: fixed local space-frequency capacity helps, but sparse-view observation tails remain",
        x=0.055,
        y=0.985,
        ha="left",
        fontsize=19.5,
    ...
    fig.text(
        0.055,
        0.947,
        "3,162/3,700 strict cells | +571 over v134 | 538 observation-only failures | 0/5 complete trajectories | independently recomputed | algorithm_breakthrough=false",
        fontsize=10.9,
        color="#53636b",
    )
