...
def test_complete_negative_and_actual_training():
    ...
    assert d['status']=='FAIL_OPENED_WORLD_TENSOR1840_LOTO'
    assert d['parameter_count']==1840 and d['fit_steps']==2600
    ...
    assert d['verification']['status']=='PASS_INDEPENDENT_WORLD_TENSOR1840_AUDIT'
    ...
    assert controls['fixed_world_tensor']['any_harm_cells']==0
    assert controls['normalized_bp']['any_harm_cells']==0
    assert all(controls[k]['any_harm_cells']==0 for k in ('dual_ridge','direct_field_ridge','rayset369'))
...
def test_bilingual_pages_and_current_manifest():
    for relative in ('index.html','operator-learning/index.html','operator-learning/daily-progress.html'):
        ...
        notes=soup.select('#world-tensor1840-result')
        assert len(notes)==1
        for lang in ('zh','en'):
