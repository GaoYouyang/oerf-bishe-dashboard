...
for j, (ax, label) in enumerate(zip(axes.flat, ['Field', 'Full gradient', 'Interior gradient', 'Observation'])):
    for key, name, color, marker in [('camera_precision_sum87', 'New sum precision (87)', '#b65748', 'o'),
                                    ('camera_precision_product87', 'Product control (87)', '#358d80', 's'),
...
fig.suptitle('Frequency-selective camera precision: fixed-budget accuracy gate failed', fontsize=14)
