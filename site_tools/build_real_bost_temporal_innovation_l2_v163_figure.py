#!/usr/bin/env python3
"""Build the public v163 temporal-persistence decision matrix."""
...
OUTPUT = ROOT / "assets/figures/real_bost_temporal_innovation_l2_v163.png"
WIDTH = 2520
HEIGHT = 1180
...
def main() -> int:
    ...
    rows = {
        (row["time"], row["camera_count"]): row
        for row in payload["primary"]["strata"]
    }
    ...
    _text(
        draw,
        (WIDTH / 2, 62),
        "v163 one-sided temporal coefficient persistence",
        size=44,
        fill="#17232d",
        bold=True,
        anchor="mm",
    ...
    _text(
        draw,
        (WIDTH / 2, 1077),
        "Independent verdict: 7/12 pass; temporal persistence amplifies sparse-view gradient tails.",
        size=25,
        fill="#7a3f2d",
        bold=True,
        anchor="mm",
    ...
    _text(
        draw,
        (WIDTH / 2, 1112),
        "t=0.75, five cameras: 0.939 / 1.359 | 28/28 checks pass | mechanism closed",
        size=21,
        fill="#66737e",
        anchor="mm",
    )
